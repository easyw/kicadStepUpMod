...
def getDXFlibs():
    ...
    `_dxfLibrary`, `_dxfColorMap`, `_dxfReader`,
    ...
    try:
        ...
        try:
            ...
            FCC.PrintWarning("import _dxfReader not found.\n")
            libsok = False
    except ImportError:
        libsok = False
        FCC.PrintWarning("DXF libraries not found.\n")
                    ...
                    if index:
                        ...
                        if (l.color == 7) and dxfBrightBackground:
                            ...
                            if isinstance(l.color, int):
                                if l.color > 0:
                                    return _dxfColorMap.color_map[l.color]
    return [0.0, 0.0, 0.0]
    ...
    if dxfGetColors and dxfobj and hasattr(dxfobj, "color_index"):
        if hasattr(obj.ViewObject, "TextColor"):
            if dxfobj.color_index == 256:
                ...
                cm = _dxfColorMap.color_map[dxfobj.color_index]
...
def processdxf(document, filename, getShapes=False, reComputeFlag=True):
    ...
    global drawing, use_dxf_internal
    if not _dxfReader:
        getDXFlibs()
        readPreferences()
        if use_dxf_internal:
            print ("dxfUseLegacyImporter",dxfUseLegacyImporter)
            dxfUseLegacyImporter = True # p.GetBool("dxfUseLegacyImporter", False)
            if forcePrefs:
                setDefPreferences()
            print ("dxfUseLegacyImporter",dxfUseLegacyImporter)
    FCC.PrintMessage("opening " + filename + "...\n")
    drawing = _dxfReader.readDXF(filename)
    global layers
    layers = []
    global doc
    doc = document
    ...
    if hasattr(drawing, "tables"):
        for table in drawing.tables.get_type("table"):
            for layer in table.get_type("layer"):
                name = layer.name
                color = tuple(_dxfColorMap.color_map[layer.color])
                drawstyle = "Solid"
                lt = rawValue(layer, 6)
...
def open(filename, forcePrefs=None):
    ...
    if forcePrefs:
        setDefPreferences()
    dxfUseLegacyImporter = True # p.GetBool("dxfUseLegacyImporter", False)
    print("use dxfUseLegacyImporter:",dxfUseLegacyImporter)
...
def insert(filename, docname, forcePrefs=None):
    ...
    readPreferences()
    if forcePrefs:
        print("forced Tracks import setting")
        setDefPreferences()
    dxfUseLegacyImporter = True # p.GetBool("dxfUseLegacyImporter", False)
    print("use dxfUseLegacyImporter:",dxfUseLegacyImporter)
    try:
...
def getBlock(sh, obj, lwPoly=False):
    ...
    It creates a `block` object using `_dxfLibrary.Block`,
    ...
    """
    block = _dxfLibrary.Block(name=obj.Name, layer=getStrGroup(obj))
    writeShape(sh, obj, block, lwPoly)
    return block
        ...
        if (len(wire.Edges) == 1) and (DraftGeomUtils.geomType(wire.Edges[0]) == "Circle"):
            ...
            if center is not None:
                if len(wire.Edges[0].Vertexes) == 1:  # circle
                    dxfobject.append(_dxfLibrary.Circle(center, radius,
                                                       color=color,
                                                       layer=layer))
                else:  # arc
                    dxfobject.append(_dxfLibrary.Arc(center, radius,
                                                    ang1, ang2, color=color,
                                                    layer=layer))
        else:
            if lwPoly:
                if hasattr(_dxfLibrary, "LwPolyLine"):
                    dxfobject.append(_dxfLibrary.LwPolyLine(getWire(wire, nospline, asis=asis),
                                                           [0.0, 0.0],
                    ...
                    FCC.PrintWarning("LwPolyLine support not found. "
                                     "Please delete _dxfLibrary.py "
                                     "from your FreeCAD user directory "
                                     "to force auto-update\n")
            else:
                dxfobject.append(_dxfLibrary.PolyLine(getWire(wire, nospline, lw=False, asis=asis),
                                                     [0.0, 0.0, 0.0],
                                                     int(DraftGeomUtils.isReallyClosed(wire)),
                                                     color=color,
                                                     layer=layer))
        ...
        for edge in loneedges:
            ...
            if (DraftGeomUtils.geomType(edge) in ["BSplineCurve",
                ...
                if (len(edge.Vertexes) == 1) and (edge.Curve.isClosed()) and (edge.Area > 0):
                    ...
                    if c:
                        dxfobject.append(_dxfLibrary.Circle(DraftVecUtils.tup(c.Curve.Center),
                                                           c.Curve.Radius,
                                                           color=color,
                                                           layer=layer))
                else:
                    ...
                    dxfobject.append(_dxfLibrary.PolyLine(points,
                                                         [0.0, 0.0, 0.0],
                                                         0, color=color,
                                                         layer=layer))
                ...
                if center is not None:
                    ...
                    if len(edge.Vertexes) == 1:  # circles
                        dxfobject.append(_dxfLibrary.Circle(center,
                                                           radius,
                                                           color=color,
                                                           layer=layer))
                    else:  # arcs
                        dxfobject.append(_dxfLibrary.Arc(center,
                                                        radius,
                                                        ang1, ang2,
                                                        color=getACI(ob),
                                                        layer=layer))
                ...
                if FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetBool("DiscretizeEllipses", True):
                    ...
                    if hasattr(_dxfLibrary, "Ellipse"):
                        ...
                        dxfobject.append(_dxfLibrary.Ellipse(center=center,
                                                            majorAxis=major,
                                                            normalAxis=norm,
                ...
                if len(edge.Vertexes) > 1:
                    ...
                    dxfobject.append(_dxfLibrary.Line([DraftVecUtils.tup(ve1),
...
def writeMesh(ob, dxf):
    ...
    dxf.append(_dxfLibrary.PolyLine([points, faces],
                                   [0.0, 0.0, 0.0],
                                   64, color=getACI(ob),
                                   layer=getGroup(ob)))
        ...
        if (len(exportList) == 1) and (Draft.getType(exportList[0]) == "ArchSectionView"):
            ...
            for ob in exportLayers:
                if ob.Label != "0":  # dxflibrary already creates it
                    ...
                    #       getACI(ob), ltype)
                    dxf.layers.append(_dxfLibrary.Layer(name=ob.Label,
                                                       color=getACI(ob),
                                                       lineType=ltype))
            ...
            for ob in exportList:
                ...
                if obtype == "PanelSheet":
                    ...
                    p1 = p2 + lspc
                    dxf.append(_dxfLibrary.Text(t1, p1, height=f1,
                    ...
                    if t2:
                        ...
                        dxf.append(_dxfLibrary.Text(t2, p1.add(ofs), height=f1,
                                                   color=getACI(ob, text=True),
                                                   style='STANDARD',
                                                   layer=getStrGroup(ob)))
                    ...
                    if not axes:
                        continue
                    for ax in axes:
                        dxf.append(_dxfLibrary.Line([ax[0],
                                                    ax[1]],
                                                    color=getACI(ob),
                                                    layer=getStrGroup(ob)))
                    h = 1
                    if FreeCAD.GuiUp:
                        ...
                        for text in vobj.Proxy.getTextData():
                            ...
                            dxf.append(_dxfLibrary.Text(text[0],
                                                       pos,
                                                       height=h,
                                                       color=getACI(ob),
                                                       style='STANDARD',
                        ...
                        for shape in vobj.Proxy.getShapeData():
                            if hasattr(shape,"Curve") and isinstance(shape.Curve,Part.Circle):
                                dxf.append(_dxfLibrary.Circle(shape.Curve.Center,
                                                             shape.Curve.Radius,
                                                             color=getACI(ob),
                                                             layer=getStrGroup(ob)))
                            else:
                                if lwPoly:
                                    ...
                                    dxf.append(_dxfLibrary.LwPolyLine(points,
                                                                     [0.0, 0.0],
                                                                     1,
                                                                     color=getACI(ob),
                                                                     layer=getGroup(ob)))
                                    ...
                                    dxf.append(_dxfLibrary.PolyLine(points,
                                                                   [0.0, 0.0, 0.0],
                                                                   1,
                                                                   color=getACI(ob),
                                                                   layer=getGroup(ob)))
                            ...
                            if sh.ShapeType == 'Compound':
                                if len(sh.Wires) == 1:
                                    ...
                                    if len(sh.Wires[0].Edges) == len(sh.Edges):
                                        ...
                                        dxf.append(_dxfLibrary.Insert(name=ob.Name.upper(),
                                                                     color=getACI(ob),
                                                                     layer=getStrGroup(ob)))
                                else:
                    ...
                    # old-style texts
                    # temporary - as _dxfLibrary doesn't support mtexts well,
                    ...
                    for text in ob.LabelText:
                        ...
                        if gui:
                            ...
                            height = 1
                        dxf.append(_dxfLibrary.Text(text, point, height=height,
                                                   color=getACI(ob, text=True),
                                                   style='STANDARD',
                                                   layer=getStrGroup(ob)))
                    ...
                    for text in ob.Text:
                        ...
                        dxf.append(_dxfLibrary.Text(text,
                                                   point,
                                                   height=height * 0.8,
                                                   rotation=rotation,
                                                   color=getACI(ob, text=True),
                    ...
                    dxf.append(_dxfLibrary.Dimension(pbase,
                                                    p1, p2,
                                                    color=getACI(ob),
                                                    layer=getStrGroup(ob)))
...
def setDefPreferences():
    """set temporary the preferences of the this module from the parameter database.
    ...
    global dxfUseLegacyImporter, dxfExportBlocks, dxfScaling
    global dxfUseLegacyExporter
    dxfCreatePart = True # p.GetBool("dxfCreatePart", True)
    dxfCreateDraft = False # p.GetBool("dxfCreateDraft", False)
    dxfCreateSketch = False # p.GetBool("dxfCreateSketch", False)
    # dxfDiscretizeCurves = p.GetBool("DiscretizeEllipses", True)
    dxfStarBlocks = False # p.GetBool("dxfstarblocks", False)
    dxfMakeBlocks = False # p.GetBool("groupLayers", False)
    dxfJoin = True # p.GetBool("joingeometry", False)
    # dxfRenderPolylineWidth = p.GetBool("renderPolylineWidth", False)
    # dxfImportTexts = p.GetBool("dxftext", False)
    dxfImportLayouts = False # p.GetBool("dxflayouts", False)
    # dxfImportPoints = False # p.GetBool("dxfImportPoints", False)
    dxfImportHatches = False # p.GetBool("importDxfHatches", False)
    dxfUseStandardSize = False # p.GetBool("dxfStdSize", False)
    #dxfGetColors = p.GetBool("dxfGetOriginalColors", False)
    dxfUseDraftVisGroups = False # p.GetBool("dxfUseDraftVisGroups", True)
    dxfFillMode = True # p.GetBool("fillmode", True)
    dxfUseLegacyImporter = True # p.GetBool("dxfUseLegacyImporter", False)
    # dxfUseLegacyExporter = p.GetBool("dxfUseLegacyExporter", False)
    dxfBrightBackground = isBrightBackground()
    dxfDefaultColor = getColor()
